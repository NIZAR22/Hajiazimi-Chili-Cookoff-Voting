...
// Database file is located at backend/chili-cookoff.db
const dbPath = path.join(__dirname, "chili-cookoff.db");
...
try {
  ...
  // Extract just the SQL part (remove the db.exec wrapper)
  const sqlCommands = schema.match(/db\.exec\(`([\s\S]*)`\);/)[1];
  db.exec(sqlCommands);
...
// Important: middleware order matters for proper request processing
app.use(cors());
app.use(express.json());

// Request logger - logs all incoming requests with timestamp and body
...
app.get("/", (req, res) => {
  try {
    // Test database connection by counting chilis
